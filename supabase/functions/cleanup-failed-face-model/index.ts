...
async function deleteS3FaceModelFolder(faceModelId: string): Promise<{ success: boolean; deletedCount: number; errors: any[] }> {
  ...
  try {
    ...
    if (deletedCount > 0) {
      console.log(`✅ Deleted ${deletedCount} S3 objects for face model: ${faceModelId}`);
    }
...
serve(async (req) => {
  ...
  try {
    ...
    const { face_model_id, user_id, reason = 'Manual cleanup' } = body;

    // Validate required fields
    if (!face_model_id || !user_id) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: face_model_id, user_id' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`🧹 Starting cleanup for face model: ${face_model_id}, reason: ${reason}`);
    ...
    try {
      ...
      if (imagesDeleteError) {
        ...
        console.log(`✅ Deleted ${imageCount || 0} image records for face model: ${face_model_id}`);
      }
    } catch (error) {
    ...
    // 3. Delete any training jobs for this face model
    try {
      ...
      if (jobsDeleteError) {
        ...
        console.log(`✅ Deleted ${jobCount || 0} training job records for face model: ${face_model_id}`);
      }
    } catch (error) {
    ...
    // 4. Delete the face model record
    try {
      ...
      if (faceModelDeleteError) {
        results.face_model_cleanup = { success: false, error: faceModelDeleteError.message };
        console.error('Failed to delete face model record:', faceModelDeleteError);
      } else {
        results.face_model_cleanup = { success: true, error: null };
        console.log(`✅ Deleted face model record: ${face_model_id}`);
      }
    } catch (error) {
      results.face_model_cleanup = { success: false, error: error.message };
      console.error('Error deleting face model record:', error);
    }

    // Determine overall success
    const overallSuccess = results.s3_cleanup.success && 
                          results.images_cleanup.success && 
                          results.face_model_cleanup.success &&
                          results.training_jobs_cleanup.success;

    console.log(`🧹 Cleanup ${overallSuccess ? 'completed successfully' : 'completed with errors'} for face model: ${face_model_id}`);
    ...
    console.error('Face model cleanup error:', error);
    return new Response(
      JSON.stringify({ 
        success: false,
