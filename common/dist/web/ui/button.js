...
const buttonVariants = ({ variant = 'primary', size = 'md', className, loading, } = {}) => {
    return cn(styles.base, variant && styles[variant], size && styles[`size-${size}`], loading && styles.loading, className);
};
const Button = React.forwardRef(({ className, variant = 'primary', size = 'md', asChild = false, loading = false, children, disabled, icon, ...props }, ref) => {
    ...
    React.useEffect(() => {
        ...
        const animate = () => {
            ...
            if (Math.abs(state.targetLabelX - state.currentLabelX) > 0.01 ||
    ...
    return (_jsxs(Comp, { "data-slot": "button", className: cn(buttonVariants({ variant, size, className, loading }), styles.btn), ref: combinedRef, disabled: disabled || loading, ...props, children: [loading && (_jsx("svg", { className: styles.spinner, viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", role: "img", "aria-label": "Loading", "aria-hidden": "true", children: _jsx("circle", { className: styles.spinnerCircle, cx: "12", cy: "12", r: "10", fill: "none", strokeWidth: "3" }) })), icon && (_jsx("span", { ref: iconRef, className: styles.btnIcon, children: icon })), _jsx("span", { ref: labelRef, className: styles.btnLabel, children: children })] }));
});
