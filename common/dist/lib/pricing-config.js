...
export async function fetchSubscriptionTiers(supabaseClient) {
    const { data, error } = await supabaseClient.from('subscriptions').select('*');
    if (error)
        throw error;
    return data;
}
export async function fetchCreditPacks(supabaseClient) {
    const { data, error } = await supabaseClient.from('credit_packs').select('*');
    if (error)
        throw error;
    return data;
}
export async function fetchCreditCosts(supabaseClient) {
