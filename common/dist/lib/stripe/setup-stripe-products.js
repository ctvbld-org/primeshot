...
async function updatePricingFile(subscriptionResults, creditPackResults) {
    ...
    if (fs.existsSync(stripeRefPath)) {
        ...
        const subsEntries = subscriptionResults.map(result => `      ${result.tier}: { product: '${result.product}', monthly: '${result.monthlyPrice}', yearly: '${result.yearlyPrice || result.monthlyPrice}' }`).join(',\n');
        const packsEntries = creditPackResults.map(result => `      ${result.pack}: { product: '${result.product}', price: '${result.price}' }`).join(',\n');
