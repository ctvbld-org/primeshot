...
function getLanguageFromAcceptLanguage(acceptLanguage?: string): string {
  ...
  for (const { code } of languages) {
    ...
    if (SUPPORTED_LANGUAGES.includes(langPart as any)) {
      return langPart;
    }
...
export function getLanguageFromCookies(cookieString?: string, acceptLanguage?: string): string {
...
export function initServerI18n(language: string = DEFAULT_LANGUAGE) {
