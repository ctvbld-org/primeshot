      ...
      <div className={styles.bannerContainer}>
        <div className={styles.bannerLeft}>
          <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M45.2445 22.0684L27.0376 40.2753L18.7617 31.9994" stroke="white" strokeWidth="4.41379" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          <h2 className={styles.bannerTitle}>
            <span>{tp('subscription.banner.titleStart')}</span>{' '}
            <span className={styles.bannerTitleBold}>{tp('subscription.banner.titleNumber')}</span>
          </h2>
        </div>
        <div className={styles.bannerRight}>
          <p className={styles.bannerDescription}>
            {tp('subscription.banner.description.start')}{' '}
            <span style={{ backgroundColor: '#FF491C', color: '#fff', padding: '4px 6px', borderRadius: '3px 0 0 3px', whiteSpace: 'nowrap' }}>
              {tp('subscription.banner.description.lifetime')}
            </span>
            <span style={{ backgroundColor: '#FFF', color: '#FF491C', padding: '4px 6px', borderRadius: '0 3px 3px 0', whiteSpace: 'nowrap' }}>
              {' '}{tp('subscription.banner.description.annualDiscount')}
            </span>{' '}
            {tp('subscription.banner.description.annualWord')} {tp('subscription.banner.description.or')}{' '}
            <span style={{ backgroundColor: '#FFF', color: '#FF491C', padding: '4px 6px', borderRadius: '3px', whiteSpace: 'nowrap' }}>
              {tp('subscription.banner.description.monthlyDiscount')}
            </span>{' '}
            {tp('subscription.banner.description.monthlyWord')}, {tp('subscription.banner.description.end')}
          </p>
