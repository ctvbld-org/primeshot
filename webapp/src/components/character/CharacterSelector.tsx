...
interface CharacterWithTraining {
  id: string;
  name: string;
...
export function CharacterSelector({ className, onModelSelected, refreshTrigger }: CharacterSelectorProps) {
  const { user } = useAuth();
  ...
  // Character training cost from DB
  const characterTrainingCost = getCharacterTrainingCost(creditCosts);
  
  // Calculate remaining character trainings
  const remainingCharacterTrainings = useMemo(() => {
  ...
  const { getUserCharacters, deleteCharacter } = useCharactersApi();
  const [characters, setCharacters] = useState<CharacterWithTraining[]>([]);
  ...
  const selectedModel = useMemo(() => 
    characters.find(model => model.id === selectedModelId),
    [characters, selectedModelId]
  );
  ...
  const hasReachedCharacterLimit = useMemo(() => {
  ...
  useEffect(() => {
    if (characters.length > 0 && !selectedModelId) {
      ...
      try {
        ...
        if (savedSelection) {
          ...
          if (characters.find(m => m.id === modelId)) {
      ...
      const firstModel = characters[0];
      setSelectedModelId(firstModel.id);
      onModelSelected?.(firstModel.id);
    }
  }, [characters, selectedModelId, onModelSelected]);
  ...
  const getTrainingJobIds = useCallback(async (models: CharacterWithTraining[]) => {
    ...
    // Create mapping of character_id -> training_job_id
    const mapping: Record<string, string> = {};
    data?.forEach(job => {
      if (!mapping[job.character_id]) { // Only take the most recent job
        mapping[job.character_id] = job.id;
      }
    });
  ...
  const fetchThumbnailUrls = useCallback(async (models: CharacterWithTraining[]) => {
  ...
  const handleDeleteConfirm = useCallback(async (model: CharacterWithTraining) => {
    if (!user?.id) return;
    
    setDeletingModelId(model.id);
    
    try {
      await deleteCharacter(model.id, user.id);
      
      // Remove from local state immediately
      setCharacters(prev => prev.filter(m => m.id !== model.id));
  ...
  }, [deleteCharacter, selectedModelId, user?.id]);
  ...
  const loadCharacters = useCallback(async () => {
    ...
    try {
      const models = await getUserCharacters(user.id);
      
      // Filter out deleted models and transform to CharacterWithTraining
      const validModels: CharacterWithTraining[] = models
  ...
  }, [user?.id, getUserCharacters, getTrainingJobIds, fetchThumbnailUrls]);
  ...
  const handleTrainingComplete = useCallback((modelId: string, success?: boolean, errorMessage?: string) => {
    ...
    if (success === false) {
      ...
      setCharacters(prev => prev.map(m => (m.id === modelId ? { ...m, status: 'failed' } : m)));
      
      // Show error toast
      toast({
        title: t('character.trainingError'),
        description: errorMessage || 'Character training failed',
        variant: 'destructive'
      });
    } else {
      // Training completed successfully - optimistically set model status to ready
      setCharacters(prev => prev.map(m => (m.id === modelId ? { ...m, status: 'ready' } : m)));
    }
    ...
    dialogService.openDialog(
      <CharacterTrainingDialog 
        onComplete={(characterId) => {
          ...
          setSelectedModelId(characterId);
          onModelSelected?.(characterId);
  ...
  }, [dialogService, loadCharacters, onModelSelected, queryClient]);
  ...
  }, [createCharacterAction, creditGuard, openCharacterTrainingDialog, openSubscriptionDialog, openCreditPackDialog, setIsPopoverOpen, subscription?.plan_name]);

  // Handle delete button click
  // Handle delete confirmation
  const handleDeleteClick = useCallback(async (e: React.MouseEvent, model: CharacterWithTraining) => {
    e.stopPropagation(); // Prevent model selection
    
    const confirmed = await confirmationService.confirm({
      title: `Delete Character '${model.name}'?`,
      description: `This can't be undone. ${needsCreditsForTraining ? `You have no included character training left. Creating a new one will cost ${characterTrainingCost} credits.` : ''}`,
      confirmText: 'Delete',
      cancelText: 'Cancel',
      variant: 'danger',
      icon: 'bin'
  ...
  }, [needsCreditsForTraining, characterTrainingCost, handleDeleteConfirm]);

  const getStatusDisplay = (model: CharacterWithTraining): React.ReactNode => {
  ...
  const getImageCount = (model: CharacterWithTraining) => {
  ...
  // Fetch training job IDs for queued/training models whenever characters changes
  useEffect(() => {
    if (!characters) return;

    const fetchJobIds = async () => {
      const ids = await getTrainingJobIds(characters);
      setTrainingJobIds(ids);
    };

    fetchJobIds();
  }, [characters, getTrainingJobIds]);

  // Clean up training progress for models that are no longer training
  useEffect(() => {
    const readyModelIds = characters.filter(m => m.status === 'ready').map(m => m.id);
      ...
      <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
        <PopoverTrigger asChild>
          <button className={styles.trigger} onClick={handleButtonClick}>
            ...
            <div className={styles.textContainer}>
              ...
              <p className={styles.secondaryText}>Character</p>
            </div>
          </button>
        </PopoverTrigger>
        <PopoverContent 
        ...
        >
          <div className={styles.dropdownHeader}>
            <h3 className={styles.dropdownTitle}>Character</h3>
          </div>
            ...
            {isLoading ? (
              ...
              <div className={styles.modelOption}>
                <div className="flex items-center space-x-3">
                  ...
                  <div className={styles.modelMeta}>Loading characters...</div>
                </div>
              </div>
            ) : error ? (
              // Show error state
              ...
              <div className={styles.modelOption}>
                <p>Add up to {maxCharacters} Character{maxCharacters !== 1 ? 's' : ''}</p>
              </div>
            ) : (
              // Show characters for authenticated users with subscription
          ...
          <div className={styles.dropdownFooter} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ minWidth: 120, textAlign: 'left', fontSize: 13, color: '#b6e3e2', fontWeight: 500 }}>
              {needsCreditsForTraining ? (
                <span>{characterTrainingCost} credits</span>
              ) : (
                <span>{isNaN(remainingCharacterTrainings) ? '0' : remainingCharacterTrainings} included in plan</span>
              )}
            </div>
            <Button 
              variant="ghost" 
              ...
              disabled={createCharacterAction.type === 'limit_reached'}
            >
              {createCharacterAction.message}
            </Button>
          </div>
        </PopoverContent>
      </Popover>
       ...
       ['queued', 'training'].includes(selectedModel.status || '') && (
        <div className="flex space-x-1">
          ...
          <div className="text-sm text-gray-400">
            {selectedModel.status === 'training' ? (
              <><span>About </span><span>{trainingProgress[selectedModel.id]?.getEstimatedTimeRemaining?.()}</span><span> remaining</span></>
            ) : (
