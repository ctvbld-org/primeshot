...
export interface FaceModelTrainingStatus {
...
export function useFaceModelTrainingStatus(activeTrainingCount: number = 0): FaceModelTrainingStatus {
  ...
  return useMemo(() => {
    ...
    const included = subscription.face_model_training_included ?? 0;
    const used = subscription.face_model_training_used ?? 0;
